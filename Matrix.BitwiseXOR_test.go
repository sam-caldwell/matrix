...
func TestMatrix_XOR(t *testing.T) {
	...
	t.Run("Given 1's integer matrix A, B, perform C=A XOR B without error", func(t *testing.T) {
		const (
			rowSize = 5
			colSize = 5
			valueA  = 1
		...
		t.Run("C=A XOR B.", func(t *testing.T) {
			var err error
			if C, err = A.XOR(B); err != nil {
				t.Fatal(err)
			}
	...
	t.Run("Given mixed integer matrix A, B, perform C=A XOR B without error", func(t *testing.T) {
		const (
			rowSize = 5
			colSize = 5
			valueA  = 0
	...
	t.Run("Given float32 matrix A, B, perform C=A XOR B, expect UnsupportedType", func(t *testing.T) {
		const (
			rowSize = 5
			colSize = 5
			valueA  = float32(1.0)
		...
		t.Run("C=A XOR B.  Expect Error", func(t *testing.T) {
			var err error
			if C, err = A.XOR(B); err == nil {
	...
	t.Run("Given float64 matrix A, B, perform C=A XOR B, expect UnsupportedType", func(t *testing.T) {
		const (
			rowSize = 5
			colSize = 5
			valueA  = 1.0
	...
	t.Run("Given an integer matrix A, B of diff sizes, C=A XOR B them, expect error", func(t *testing.T) {
		var (
			A *Matrix[int]
			B *Matrix[int]
			C *Matrix[int]
